...
tracker:
  kind: linear
  project_slug: "odyssey-0c79b11b75ea"
  active_states:
    - Todo
    - In Progress
    - AI Review
...
polling:
  interval_ms: 5000
workspace:
  root: ~/code/odyssey-workspaces
  source_repo: ~/code/odyssey
hooks:
  after_create: |
...
5.  Ensure the workpad includes a compact environment stamp at the top as a code fence line:
    ...
    - Example: `devbox-01:/home/dev-user/code/odyssey-workspaces/MT-32@7bdde33bc`
...
- If truly blocked by missing tools/auth, record a blocker brief in the workpad comment with:
  - what is missing,
  - why it blocks required acceptance/validation,
  - exact action needed to unblock.
- Keep the brief concise and action-oriented; do not add extra top-level comments outside the workpad.
- Do NOT stop work or change issue state for blockers. Continue with whatever is possible and document the gap.
...
8.  Attach PR URL to the issue (prefer attachment; use the workpad comment only if attachment is unavailable).
    - Ensure the GitHub PR has label `odyssey` (add it if missing).
...
- Required PR metadata is present (`odyssey` label).
