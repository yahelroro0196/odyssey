defmodule OdysseyElixir.WorkspaceAndConfigTest do
  use OdysseyElixir.TestSupport
  alias Ecto.Changeset
  alias OdysseyElixir.Config.Schema
  alias OdysseyElixir.Config.Schema.{Codex, StringOrMap}
  ...
  test "workspace bootstrap can be implemented in after_create hook" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hook-bootstrap-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace path is deterministic per issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-deterministic-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace reuses existing issue directory without deleting local changes" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-reuse-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace replaces stale non-directory paths" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-stale-path-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace} = OdysseyElixir.PathSafety.canonicalize(stale_workspace)
  ...
  test "workspace rejects symlink escapes under the configured root" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-symlink-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_outside_root} = OdysseyElixir.PathSafety.canonicalize(outside_root)
      assert {:ok, canonical_workspace_root} = OdysseyElixir.PathSafety.canonicalize(workspace_root)
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-root-symlink-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace} =
               OdysseyElixir.PathSafety.canonicalize(Path.join(actual_root, "MT-LINK"))
  ...
  test "workspace remove rejects the workspace root itself with a distinct error" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-root-remove-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace_root} =
               OdysseyElixir.PathSafety.canonicalize(workspace_root)
  ...
  test "workspace surfaces after_create hook failures" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hook-failure-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace surfaces after_create hook timeouts" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hook-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace creates an empty directory when no bootstrap hook is configured" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-workspace-empty-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace} = OdysseyElixir.PathSafety.canonicalize(workspace)
  ...
  test "workspace removes all workspaces for a closed issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-issue-workspace-cleanup-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace cleanup handles missing workspace root" do
    missing_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-missing-workspaces-#{System.unique_integer([:positive])}"
      )
  ...
  test "linear client paginates issue state fetches by id beyond one page" do
    ...
    assert query =~ "OdysseyLinearIssuesById"
  ...
  test "workspace remove returns error information for missing directory" do
    random_path =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-missing-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace hooks support multiline YAML scripts and run at lifecycle boundaries" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hooks-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hooks-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails with large output" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hooks-large-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook times out" do
    previous_timeout = Application.get_env(:odyssey_elixir, :workspace_hook_timeout_ms)

    on_exit(fn ->
      if is_nil(previous_timeout) do
        Application.delete_env(:odyssey_elixir, :workspace_hook_timeout_ms)
      else
        Application.put_env(:odyssey_elixir, :workspace_hook_timeout_ms, previous_timeout)
      end
    end)

    Application.put_env(:odyssey_elixir, :workspace_hook_timeout_ms, 10)

    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-workspace-hooks-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.workspace.root == Path.join(System.tmp_dir!(), "odyssey_workspaces")
    ...
    assert {:ok, canonical_default_workspace_root} =
             OdysseyElixir.PathSafety.canonicalize(Path.join(System.tmp_dir!(), "odyssey_workspaces"))
    ...
    explicit_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-explicit-sandbox-root-#{System.unique_integer([:positive])}"
      )
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    workspace_root = Path.join("/tmp", "odyssey-workspace-root")
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    assert settings.workspace.root == Path.join(System.tmp_dir!(), "odyssey_workspaces")
  ...
  test "schema resolves sandbox policies from explicit and default workspaces" do
    ...
    assert Schema.resolve_turn_sandbox_policy(%Schema{
             ...
             "type" => "workspaceWrite",
             "writableRoots" => [Path.expand(Path.join(System.tmp_dir!(), "odyssey_workspaces"))],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
  ...
  test "schema keeps workspace roots raw while sandbox helpers expand only for local use" do
    assert {:ok, settings} =
             Schema.parse(%{
               workspace: %{root: "~/.odyssey-workspaces"},
               codex: %{}
             })

    assert settings.workspace.root == "~/.odyssey-workspaces"

    assert Schema.resolve_turn_sandbox_policy(settings) == %{
             "type" => "workspaceWrite",
             "writableRoots" => [Path.expand("~/.odyssey-workspaces")],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
    ...
    assert remote_policy == %{
             "type" => "workspaceWrite",
             "writableRoots" => ["~/.odyssey-workspaces"],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
  ...
  test "runtime sandbox policy resolution passes explicit policies through unchanged" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-runtime-sandbox-#{System.unique_integer([:positive])}"
      )
  ...
  test "path safety returns errors for invalid path segments" do
    ...
    assert {:error, {:path_canonicalize_failed, ^expanded_path, :enametoolong}} =
             OdysseyElixir.PathSafety.canonicalize(path)
  end
  ...
  test "runtime sandbox policy resolution defaults when omitted and ignores workspace for explicit policies" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-runtime-sandbox-branches-#{System.unique_integer([:positive])}"
      )
  ...
  test "remote workspace lifecycle uses ssh host aliases from worker config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-remote-workspace-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      workspace_root = "~/.odyssey-remote-workspaces"
      workspace_path = "/remote/home/.odyssey-remote-workspaces/MT-SSH-WS"
      ...
      File.write!(fake_ssh, """
      #!/bin/sh
      trace_file="${SYMP_TEST_SSH_TRACE:-/tmp/odyssey-fake-ssh.trace}"
      printf 'ARGV:%s\\n' "$*" >> "$trace_file"

      case "$*" in
        *"__ODYSSEY_WORKSPACE__"*)
          printf '%s\\t%s\\t%s\\n' '__ODYSSEY_WORKSPACE__' '1' '#{workspace_path}'
          ;;
      esac
      ...
      assert trace =~ "__ODYSSEY_WORKSPACE__"
      assert trace =~ "~/.odyssey-remote-workspaces/MT-SSH-WS"
