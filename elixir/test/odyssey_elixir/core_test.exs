defmodule OdysseyElixir.CoreTest do
  ...
  test "OdysseyElixir.start_link delegates to the orchestrator" do
    write_workflow_file!(Workflow.workflow_file_path(), tracker_kind: "memory")
    Application.put_env(:odyssey_elixir, :memory_tracker_issues, [])
    orchestrator_pid = Process.whereis(OdysseyElixir.Orchestrator)

    on_exit(fn ->
      if is_nil(Process.whereis(OdysseyElixir.Orchestrator)) do
        case Supervisor.restart_child(OdysseyElixir.Supervisor, OdysseyElixir.Orchestrator) do
          {:ok, _pid} -> :ok
    ...
    if is_pid(orchestrator_pid) do
      assert :ok = Supervisor.terminate_child(OdysseyElixir.Supervisor, OdysseyElixir.Orchestrator)
    end

    assert {:ok, pid} = OdysseyElixir.start_link()
    assert Process.whereis(OdysseyElixir.Orchestrator) == pid
  ...
  test "non-active issue state stops running agent without cleaning workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-nonactive-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "terminal issue state stops running agent and cleans workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-terminal-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "missing running issues stop active agents without cleaning the workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-missing-running-reconcile-#{System.unique_integer([:positive])}"
      )

    previous_memory_issues = Application.get_env(:odyssey_elixir, :memory_tracker_issues)
    ...
    try do
      ...
      assert {:ok, workspace} =
               OdysseyElixir.PathSafety.canonicalize(Path.join(test_root, issue_identifier))
  ...
  defp restore_app_env(key, nil), do: Application.delete_env(:odyssey_elixir, key)
  defp restore_app_env(key, value), do: Application.put_env(:odyssey_elixir, key, value)
  ...
  test "prompt builder reports workflow load failures separately from template parse errors" do
    original_workflow_path = Workflow.workflow_file_path()
    workflow_store_pid = Process.whereis(OdysseyElixir.WorkflowStore)

    on_exit(fn ->
      Workflow.set_workflow_file_path(original_workflow_path)

      if is_pid(workflow_store_pid) and is_nil(Process.whereis(OdysseyElixir.WorkflowStore)) do
        Supervisor.restart_child(OdysseyElixir.Supervisor, OdysseyElixir.WorkflowStore)
      end
    end)

    assert :ok = Supervisor.terminate_child(OdysseyElixir.Supervisor, OdysseyElixir.WorkflowStore)
  ...
  test "agent runner keeps workspace after successful codex run" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-agent-runner-retain-workspace-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner forwards timestamped codex updates to recipient" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-agent-runner-updates-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner surfaces ssh startup failures instead of silently hopping hosts" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-agent-runner-single-host-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      case "$*" in
        *worker-a*"__ODYSSEY_WORKSPACE__"*)
          printf '%s\\n' 'worker-a prepare failed' >&2
          exit 75
          ;;
        *worker-b*"__ODYSSEY_WORKSPACE__"*)
          printf '%s\\t%s\\t%s\\n' '__ODYSSEY_WORKSPACE__' '1' '/remote/home/.odyssey-remote-workspaces/MT-SSH-FAILOVER'
          exit 0
          ;;
        *)
          exit 0
  ...
  test "agent runner continues with a follow-up turn while the issue remains active" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-agent-runner-continuation-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner stops continuing once agent.max_turns is reached" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-agent-runner-max-turns-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server starts with workspace cwd and expected startup command" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-app-server-args-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server startup command supports codex args override from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-app-server-custom-args-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server startup payload uses configurable approval and sandbox settings from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "odyssey-elixir-app-server-policy-overrides-#{System.unique_integer([:positive])}"
      )
