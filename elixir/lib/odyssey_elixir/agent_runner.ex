defmodule OdysseyElixir.AgentRunner do
  @moduledoc """
  ...
  defp run_agent_turns(workspace, issue, agent_update_recipient, opts, worker_host) do
    ...
    backend = Config.agent_backend(role)

    with {:ok, session} <- backend.start_session(workspace, worker_host: worker_host, role: role) do
      ctx = %{backend: backend, session: session, workspace: workspace}

      try do
        do_run_agent_turns(ctx, issue, agent_update_recipient, opts, issue_state_fetcher, 1, max_turns)
      after
  ...
  defp do_run_agent_turns(ctx, issue, agent_update_recipient, opts, issue_state_fetcher, turn_number, max_turns) do
