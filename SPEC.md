...
Odyssey is a long-running automation service that continuously reads work from an issue tracker
...
- Odyssey is a scheduler/runner and tracker reader.
...
Odyssey is easiest to port when kept in these layers:
...
- `root` (path string or `$VAR`)
  - Default: `<system-temp>/odyssey_workspaces`
...
- `workspace.root`: path, default `<system-temp>/odyssey_workspaces`
...
```json
{"id":1,"method":"initialize","params":{"clientInfo":{"name":"odyssey","version":"1.0"},"capabilities":{}}}
...
- Purpose: execute a raw GraphQL query or mutation against Linear using Odyssey's configured
...
- Reuse the configured Linear endpoint and auth from the active Odyssey workflow/runtime config; do
...
Odyssey does not require first-class tracker write APIs in the orchestrator.
...
- `GET /api/v1/<issue_identifier>`
  ...
  - Suggested response shape:
    ...
    ```json
    {
      ...
      "status": "running",
      "workspace": {
        "path": "/tmp/odyssey_workspaces/MT-649"
      },
      "attempts": {
        "restart_count": 1,
      ...
      "logs": {
        "codex_session_logs": [
          {
            "label": "latest",
            "path": "/var/log/odyssey/codex/MT-649/latest.log",
            "url": null
          }
        ]
      },
...
- Optional `linear_graphql` client-side tool extension exposes raw Linear GraphQL access through the
  app-server session using configured Odyssey auth.
...
This appendix describes a common extension profile in which Odyssey keeps one central
